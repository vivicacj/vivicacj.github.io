 ...
 * v11.6 - RESTORED DATA & INTEGRATED LOGIC
 * * CRITICAL FIXES:
 * 1. Separated Main Map data from Geo Framework data to prevent data loss.
 * 2. Restored full rendering for non-Geo archetypes (Algebra, Stats, etc.).
 * 3. Kept all v11.5 features (English UI, Stacked Layout, Wizard, Consistent Styling).
 */
...
let allSOPs = []; // Contains ALL SOPs (Algebra, Stats, Geo, etc.)
...
function injectStyles() {
    ...
    style.innerHTML = `
        /* Universal Styles */
        .sop-section { margin-top: 1rem; padding: 1.5rem; background: rgba(255,255,255,0.03); border-radius: 8px; border: 1px solid rgba(255,255,255,0.05); position: relative; overflow: hidden; }
        .modal-sub-branch-title { color: var(--accent, #06b6d4); font-weight: 700; margin-top: 2rem; margin-bottom: 1rem; text-transform: uppercase; font-size: 0.9em; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 0.5rem; }
        
        .pitfall-item { background: rgba(239, 68, 68, 0.15); border-left: 3px solid #ef4444; padding: 0.75rem; margin-bottom: 0.5rem; display: flex; gap: 0.75rem; border-radius: 4px; color: #fca5a5; }
        .pro-tip-item { background: rgba(16, 185, 129, 0.15); border-left: 3px solid #10b981; padding: 0.75rem; margin-bottom: 0.5rem; display: flex; gap: 0.75rem; border-radius: 4px; color: #86efac; }
        ...
        .geo-framework-section { padding: 4rem 2rem; max-width: 1600px; margin: 0 auto; position: relative; border-top: 1px solid rgba(255,255,255,0.1); margin-top: 4rem; }
        ...
        .geo-stuck-btn { position: absolute; right: 0; top: 50%; transform: translateY(-50%); background: linear-gradient(135deg, #f59e0b, #d97706); color: white; border: none; padding: 0.6rem 1.2rem; border-radius: 2rem; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 0.5rem; box-shadow: 0 4px 15px rgba(245, 158, 11, 0.4); transition: all 0.3s ease; z-index: 10; }
        .geo-stuck-btn:hover { transform: translateY(-52%) scale(1.05); box-shadow: 0 6px 20px rgba(245, 158, 11, 0.6); }
        
        .geo-instruction-banner { background: rgba(99, 102, 241, 0.1); border: 1px solid rgba(99, 102, 241, 0.3); border-radius: 1rem; padding: 1rem 1.5rem; margin-bottom: 2rem; display: flex; align-items: center; gap: 1rem; cursor: pointer; transition: all 0.3s ease; }
        ...
        .geo-layer-card { background: rgba(255,255,255,0.03); border-radius: 1rem; border: 1px solid rgba(255,255,255,0.1); overflow: hidden; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
        .geo-layer-card.expanded { border-color: var(--layer-color, rgba(255,255,255,0.2)); background: rgba(255,255,255,0.05); box-shadow: 0 10px 30px -10px rgba(0,0,0,0.5); }
        
        .geo-layer-header { padding: 1.5rem 2rem; cursor: pointer; display: flex; align-items: center; justify-content: space-between; user-select: none; background: linear-gradient(90deg, rgba(255,255,255,0.02) 0%, transparent 100%); min-height: 88px; }
        ...
        .geo-count-badge { background: rgba(0,0,0,0.3); color: var(--layer-color); padding: 0.25rem 0.75rem; border-radius: 1rem; font-size: 0.85rem; font-weight: 600; border: 1px solid var(--layer-color); min-width: 80px; text-align: center; }
        ...
        /* Mini SOP Card */
        .mini-sop { background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.05); border-radius: 0.75rem; overflow: hidden; transition: all 0.2s; cursor: pointer; display: flex; flex-direction: column; height: 100%; }
        ...
        /* Wizard Modal */
        .geo-wizard-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); backdrop-filter: blur(5px); z-index: 10000; display: none; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.3s ease; }
        .geo-wizard-overlay.active { display: flex; opacity: 1; }
        .geo-wizard-modal { background: #1e293b; border: 1px solid rgba(255,255,255,0.1); border-radius: 1.5rem; padding: 2rem; max-width: 500px; width: 90%; text-align: center; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); transform: scale(0.9); transition: transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1); }
        ...
        .geo-wizard-option { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); padding: 1rem; border-radius: 1rem; color: #e2e8f0; font-weight: 600; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: space-between; }
...
function processData() {
    // 1. RESTORE MAIN MAP DATA
    allSOPs = SOPS_DATA || [];
    if (!ARCHETYPES_DATA) {
        console.error("Missing Archetypes Data");
        return;
    }
...
function renderInPlaceContent(archetype, hasChildren) {
    ...
    // 1. Render SOPs (Required & Support)
    html += renderSopDetails(archetype);

    // 2. Render Archetype Specifics
...
function renderSopDetails(archetype) {
    ...
    const support = getList(archetype, 'supportSOPs', 'support_sops');
    if (support.length > 0) {
...
function renderArchetypeSpecifics(archetype) {
    ...
    if (microSkills.length > 0) {
        ...
        html += `<div class="sop-section" style="border-left: 3px solid var(--accent); margin-bottom: 1rem;">
            <ul style="list-style: disc; padding-left: 1.5rem; color: #e2e8f0; margin: 0;">${renderSafeListItems(microSkills)}</ul>
        </div>`;
    }
    if (pitfalls.length > 0) html += `<div style="margin-top: 1rem;">${renderSafePitfalls(pitfalls)}</div>`;
    if (tips.length > 0) html += `<div style="margin-top: 0.5rem;">${renderSafeProTips(tips)}</div>`;
    return html;
}
    ...
    <div class="mini-sop" onclick="this.classList.toggle('active')">
        ...
        <div class="mini-sop-detail">
            <div class="detail-pad">
                ...
                ${pitfalls.length > 0 ? `<div class="detail-label">Pitfalls</div>${renderSafePitfalls(pitfalls)}` : ''}
                ${tips.length > 0 ? `<div class="detail-label">Pro Tips</div>${renderSafeProTips(tips)}` : ''}
            </div>
        </div>
    </div>
    `;
...
// ==========================================
function init() {
    console.log("🚀 App Initialized v11.6 (Data Restored + Geo Logic)");
    injectStyles();
    processData();
...
// Wizard
window.openGeoWizard = () => document.getElementById('geoWizardOverlay').classList.add('active');
window.closeGeoWizard = () => document.getElementById('geoWizardOverlay').classList.remove('active');
